...
export default function CustomersApp() {
  ...
  useEffect(() => { if (localStorage.getItem("bazarek-role") === "user") { window.location.replace("/products"); return; } fetch("/api/database", { cache: "no-store" }).then((response) => response.ok ? response.json() : Promise.reject()).then((database) => { setCustomers(Array.isArray(database.customers) ? database.customers : []); setNotes(database.customerNotes || {}); setCustomerSettings(database.customerSettings || { categories: [], assignments: {} }); }).catch(() => undefined); }, []);
  ...
  const saveCustomer = async (nextCustomer: Customer) => {
    const next = customers.map((customer) => customer.id === nextCustomer.id ? nextCustomer : customer);
    setCustomers(next);
    setSelected(nextCustomer);
    try { await saveCustomerSection("customers", next); } catch { window.alert("ذخیره اطلاعات مشتری در MongoDB ناموفق بود."); }
  };
...
function CustomerDetail({ customer, note, categories, categoryId, onCategory, onClose, onSave, onUpdate }: { customer: Customer; note: string; categories: CustomerSettings["categories"]; categoryId: string; onCategory: (id: string) => void; onClose: () => void; onSave: (id: number, text: string) => void; onUpdate: (customer: Customer) => Promise<void> }) {
  const [text, setText] = useState(note);
  const [followUps, setFollowUps] = useState(() => [...(customer.followUps || [])].sort((a, b) => b.date.localeCompare(a.date, "fa")));
  ...
  const addFollowUp = async () => {
    ...
    try {
      ...
      setFollowUps([{ date: followUpDate, note: followUpNote.trim() }, ...followUps]);
      setFollowUpDate(jalaliToday());
      setFollowUpNote("");
    } catch { window.alert("ذخیره پیگیری در MongoDB ناموفق بود."); } finally { setSavingFollowUp(false); }
  };
  useEffect(() => {
    const title = document.querySelector(".fixed.inset-0.z-20 h2");
    if (!title) return;
    const form = document.createElement("form");
    form.className = "customer-phone-editor mt-4 rounded-xl border border-oxblood/10 bg-blush p-3";
    ...
    form.addEventListener("submit", (event) => { event.preventDefault(); const data = new FormData(form); void onUpdate({ ...customer, mobile: String(data.get("mobile") || "").trim(), phone: String(data.get("phone") || "").trim() }); });
    title.after(form);
    return () => form.remove();
  }, [customer, onUpdate]);
