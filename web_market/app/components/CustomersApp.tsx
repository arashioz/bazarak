...
export default function CustomersApp() {
  ...
  const importCustomers = async (event: ChangeEvent<HTMLInputElement>) => {
    ...
    try {
      ...
      setCustomers(data);
      await fetch("/api/database", { method: "PUT", headers: { "content-type": "application/json" }, body: JSON.stringify({ section: "customers", data }) });
    } catch { window.alert("فایل اکسل خوانده نشد؛ ستون «نام» را بررسی کنید."); }
  };
