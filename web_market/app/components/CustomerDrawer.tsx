...
export default function CustomerDrawer() {
  ...
  return <>
    <button type="button" onClick={() => { setOpen(true); void loadCustomers(); }} className="fixed left-4 top-[calc(50%-3.5rem)] z-30 inline-flex items-center gap-2 rounded-full bg-white px-4 py-3 text-sm font-black text-oxblood shadow-xl ring-1 ring-oxblood/15 transition hover:bg-blush"><Users size={18} /> مشتریان</button>
    {open && <div className="fixed inset-0 z-40 flex justify-end">
      ...
      <aside className="relative flex h-full w-[92vw] max-w-[420px] flex-col bg-white shadow-2xl animate-in slide-in-from-right duration-200">
        ...
        <div className="flex-1 overflow-y-auto p-3">
          {loading && <p className="rounded-lg bg-blush p-4 text-center text-sm font-bold text-oxblood">در حال دریافت مشتریان…</p>}
