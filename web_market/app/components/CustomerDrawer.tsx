...
export default function CustomerDrawer() {
  ...
  useEffect(() => { fetch("/api/database", { cache: "no-store" }).then((response) => response.ok ? response.json() : Promise.reject()).then((database) => setCustomers(Array.isArray(database.customers) ? database.customers : [])).catch(() => undefined); }, []);
